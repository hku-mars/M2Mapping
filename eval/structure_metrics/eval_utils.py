...
def eval_mesh(
    ...
    if gt_bbx_mask_on:
        ...
        trgt_bbx = pcd_trgt.get_minimal_oriented_bounding_box()
    ...
    metrics = {
        "MAE_accuracy (cm)": dist_p_mean * 100.0,
        "MAE_completeness (cm)": dist_r_mean * 100.0,
        "Chamfer_L1 (cm)": chamfer_l1 * 100.0,
        "Chamfer_L2 (cm)": chamfer_l2 * 100.0,
        "Precision [Accuracy] (%)": precision,
        "Recall [Completeness] (%)": recall,
        "F-score (%)": fscore,
        "Spacing (cm)": down_sample_res * 100.0,  # evlaution setup
        "Inlier_threshold (cm)": threshold * 100.0,  # evlaution setup
        "Outlier_truncation_acc (cm)": truncation_acc * 100.0,  # evlaution setup
        "Outlier_truncation_com (cm)": truncation_com * 100.0,  # evlaution setup
    }
    return metrics
